...
def include_Aude?(tab)
	tab.count {|j| j =~ /(aude)/i}
end

def begin_with_Maj?(tab)
	tab.count {|j| j =~ /@[A-Z]/}
end

def contain_Maj?(tab)
	tab.count {|j| j.downcase != j}
end

def num_underscore?(tab)
    tab.count {|j| j.include? '_'}
end
...
def fifty_lesser(tab)
    tab.index("@epenser")
end

def perform
    taille($journalists)
    puts include_num?($journalists)
    puts include_Aude?($journalists)
    puts begin_with_Maj?($journalists)
    puts contain_Maj?($journalists)
    puts num_underscore?($journalists)
    puts alphabetize($journalists)
    puts fifty_lesser($journalists)
end
